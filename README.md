...
- `/theme-forge wire-direction [scope]` (alias `/theme-forge wire direction`, `/theme-forge wire-design`) wires and injects the design direction aesthetic foundations (tokens, typography, master stylesheet, OOP JavaScript runtime, and static assets) directly into the Shopify theme codebase (`assets/`, `snippets/`, `layout/theme.liquid`) using a fallback-first token bridge, allowing immediate live previewing via Shopify CLI before or alongside merchant theme settings (`config/settings_schema.json`).
...
- **Clean Scaffold Pages (No Scaffold Banners)**: Missing default page prototypes are created as minimal accessible scaffolds marked cleanly via HTML attribute (`<main data-prototype-status="scaffold">`) with zero visual scaffold banners or distracting disclaimer ribbons.
